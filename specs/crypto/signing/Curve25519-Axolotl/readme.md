## Description
Test vectors for Topl implementation of WhisperSystem Curve25519-Axolotl signing routine [[1](https://en.wikipedia.org/wiki/Double_Ratchet_Algorithm)][[2](https://github.com/tgalal/python-axolotl-curve25519)][[3](https://github.com/signalapp/libsignal-protocol-java)]. These test vectors were generated on June 7, 2021 from [co.topl.crypto.signing.Curve25519.scala](https://github.com/Topl/Bifrost/blob/ef396e2faa36cfb50a6ec00961a2a021491c88a6/crypto/src/main/scala/co/topl/crypto/signatures/Curve25519.scala) at commit `ef396e2faa36cfb50a6ec00961a2a021491c88a6` with the UTF-8 encoded seeds `test1, test2, test3, test1024, testsha`.

All values below are given as hex encoded (base16) strings unless otherwise specified.

## Reference implementations
- [Scala](https://github.com/Topl/Bifrost/blob/tetra/crypto/src/test/scala/co/topl/crypto/signing/Curve25519AxolotlSpec.scala)

## Test vectors
```json
[
  {
    "description": "test vector 1 - empty message",
    "inputs": {
      "secretKey": "184F0E9851971998E732078544C96B36C3D01CEDF7CAA332359D6F1D83567054",
      "message": ""
    },
    "outputs": {
      "verificationKey": "4652486EBC271520D844E5BDDA9AC243C05DCBE7BC9B93807073A32177A6F73D",
      "signature": "AD7B2C434DE419712A55AF65D485DA4F673076D4FDBFF4730A20AA8DC1F0C05AD757F6B50674CFB622131377F29C646DF60C7148E6B8AF33850276F98D31DA0A"
    }
  },
  {
    "description": "test vector 2 - one byte message",
    "inputs": {
      "secretKey": "60303AE22B998861BCE3B28F33EEC1BE758A213C86C93C076DBE9F558C11C752",
      "message": "72"
    },
    "outputs": {
      "verificationKey": "FFBC7BA2E4C43BE03F8A7F020D0651F582AD1901C254EEBB4EC2ECB73148E50D",
      "signature": "E7D628E6A25AFF86A7A81CF60B40D0C8BCA038A75CB6BEF34B384E5D098C1F291FD7068CDBF001CAF0EB82810B0A13F0B2806E02124E416B671E51241DD4EB0F"
    }
  },
  {
    "description": "test vector 3 - two byte message",
    "inputs": {
      "secretKey": "F861A03AF4F77D870FC21E05E7E80678095C92D808CFB3B5C279EE04C74ACA53",
      "message": "af82"
    },
    "outputs": {
      "verificationKey": "59DF714EAD8FB10B68E31153AD01994117652CB3C960C6E32C57E7DEC28A5846",
      "signature": "12ED7C9B5D757C23809FF620CF0B48CF054F6FC60B8B45B50C49F78C80332D1B4F1DEF16F3270E75686CF12F661CF777EBDCFF2977078D5EEC32F0EB39D11802"
    }
  },
  {
    "description": "test vector 4 - using a large message",
    "inputs": {
      "secretKey": "30929AC0656504EF9782D0164A3603C09AA3FDCB07973948D03472A571CAB769",
      "message": ""
    },
    "outputs": {
      "verificationKey": "05C54041990DEF35C6B163F1B6DBE5194BC4139C7D3BA8F1FCF28DE3A8B07357",
      "signature": "AA9D3EF915F416A85717A00B69C4CB01514AEB9D743259974F36B9EEB429BE4039C0491C68EE3987F493BFB7D54039BA2D03DD8CBF83A1D978E0B18C4C363680"
    }
  },
  {
    "description": "test vector 5 - Using a message that is the digest from SHA512('abc') (string input to SHA utf-8 encoded)",
    "inputs": {
      "secretKey": "C8A312FBBCF8FF3213B917D4232BCA39AAE7740338791114072F07FF3692CA72",
      "message": "DDAF35A193617ABACC417349AE20413112E6FA4E89A97EA20A9EEEE64B55D39A2192992A274FC1A836BA3C23A3FEEBBD454D4423643CE80E2A9AC94FA54CA49F"
    },
    "outputs": {
      "verificationKey": "5EFBB8396F73F2663403B242347E8B72FBA96DD1E92DBA71A909076643631752",
      "signature": "DC8566EF933EEE7527E7150C5464CAA67F244BF6DCE052821B5A7424892D28798C6674E2B6671E3AB3E73B0FA8F2BC960714FF05F48DA91CD720FE7140875E87"
    }
  }
]
```